...
**Primary goal (generator improvement — reprioritized 2026-07):** make the *base generator itself* more temporally consistent (less drift, better subject/background consistency) — gains must survive in **Mode 0** (heads discarded at inference, zero overhead). Two backbone channels serve this: (a) the LSC gradient through Tap-A `h_fuse` (representation shaping — an empirically-tested hypothesis, not an a-priori guarantee), and (b) **C1 grounding, now first-class**: the DMD teacher gradient applied to the head's drafted future block, which injects *teacher* information about futures into the backbone features — the strongest a-priori backbone-improving signal (§4). **Secondary claim (drafter, free byproduct):** the heads enable *self-speculative streaming* — drafted blocks skip the 4-step denoise chain, ceiling 1.67× (K=1 alternate) and ~3× (chained, §7). The claims are architecturally decoupled (dual-tap, §3); if the regularizer channels are a wash, the drafter still ships (kill criterion in §11).
...
**Variant C — grounded lookahead (C1 now DEFAULT-ON; the rebuttal to "vacuous self-distillation").**
...
- **C1 — DMD-on-drafts (default on; primary backbone signal given the generator-improvement goal):** the head exposes a clean draft `x̂_{b+k}` (variant B: `x_t′ − σ′·v̂`; variant A: direct x0), differentiable through `h_fuse`. Splice one draft per rollout into the **detached** rollout window and run the standard DMD loss with a gradient mask on the draft slice — the bidirectional critic stays in-distribution (full 21-frame window) while gradient flows only through the draft → `h_fuse` → backbone. This says: *block b's features must imagine a future the teacher finds realistic* — teacher information, not self-consistency. Cost ≈ one extra DMD-scored window per generator step; joins the λ-scaled exit channel (`lookahead_dmddraft_exit`) so the ratio controller regulates it. Config: `grounding: dmd_on_drafts`, `grounding_weight`.
...
- **M2 (main result — go/no-go gate):** three arms from the released checkpoint, identical data/steps/seeds, ~3k generator updates (≈15k trainer steps at ratio 5), **2 seeds each**:
  ...
  - (B) +LSC Variant B, K=1, dual-tap, ratio-controlled λ, **C1 grounding on** (the primary arm for the generator-improvement goal);
  - (B′) same as B with `grounding: none` (isolates C1's contribution) — run if budget allows, else fold into M5 ablations;
  - (C) heads on **detached** features (isolates the drafter from all backbone effects — makes the causal claim clean).
  Numeric gates (Mode 0 is the headline): B's VBench total within 0.3 of A **and** dynamic degree ≥95% of A (safety); drift slope ≥15% better than A to *claim* the generator improvement; continuation diversity ≥95% of A; drafter gate for M3 = held-out draft quality such that a 20-prompt Mode-1 pilot loses ≤1.0 VBench total at measured ≥1.4× wall-clock on one A100. **The generator-improvement gates are the go/no-go for the project's primary goal**; the drafter gate independently decides M3.
